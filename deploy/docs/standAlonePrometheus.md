...
- Replace `$(CHART)` with the name of the Fluentd metrics service. `kubectl get svc -n <COLLECTION NAMESPACE>` can help with that.
- Replace `$(NAMESPACE)` with the namespace where Collection is running.
...
```
`$(CHART).$(NAMESPACE)` will be replaced by `collection-sumologic-fluentd-metrics.sumologic`
```
